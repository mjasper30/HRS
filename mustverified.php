    ...
    if(isset($_POST['submit'])){
        ...
        if($otp != $otp_code){
           ...
           <?php
            ...
            $sql="UPDATE hrs_tbluser set IsVerified = '$isverified' WHERE Email='$email'";
            if (mysqli_query($conn, $sql)) {
                echo "<script>alert('Your account is now verified!');</script>";
                 ?><script>
                       window.location.replace("signin.php");
                 </script><?php
            }else{
                echo "<script>alert('Code has an error');</script>";
            }
